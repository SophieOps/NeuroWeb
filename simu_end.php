    ...
    <body>
        ...
        <div class="row">
            ...
            <?php
                ...
                $sum_question = $_SESSION['sum_question'];
                unset($_SESSION['score']);
                unset($_SESSION['question']);
                unset($_SESSION['sum_question']);
                unset($_SESSION['theme_id']);
                unset($_SESSION['affirmation1']);
                unset($_SESSION['affirmation2']);
                unset($_SESSION['affirmation3']);
                if ($score > ($sum_question/2))
                {
                    echo '<h2> Félicitation, vous avez optenu un score de '.$score.'/'.$sum_question.'.</h2>';
                }elseif($score == ($sum_question/2))
                {
                    echo '<h2> Vous avez optenu un score de '.$score.'/'.$sum_question.'.</h2>';
                }elseif ($score < $sum_question) {
                    echo '<h2> Attention, vous avez optenu un score de '.$score.'/'.$sum_question.'.</h2>';
                }
            ?>
